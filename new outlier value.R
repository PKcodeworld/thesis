...
for (i in 1:iterate_time){
 ...
 # }
  ...
  for (i in 1:numcol){
    ...
    Z40[index40[i]]=-1
    out = 20
    alltime[index20[i]]=out
    alltime40[index40[i]]=out
...
xx= 0:numcol
plot(xx,Hbias,col=1,type="b",pch=21,ylim=c(-0.5*N,1.5*N),xlab = "No. of outliers",ylab ="|bias|*N")
lines(xx,rep(0,numcol+1),type="l")
lines(xx,Pbias,col=2,type="b",pch=22)
lines(xx,H40bias,col=3,type="b",,pch=23)
lines(xx,P40bias,col=4,type="b",,pch=24)
legend("topleft",x.intersp=0.3,y.intersp=0.2,pch=21:24,text.width=1,legend=c("Hbias(no censor)","Pbias(no censor)","Hbias(40%censor)","Pbias(40%censor)"),col=c(1,2,3,4),bty="n")
