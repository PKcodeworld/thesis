...
for (i in 1:iterate_time){
  ...
  influH40 = hat40$minimum
  
  coxfit = coxph(Surv(alltime,del)~Z)
  influP=coxfit$coefficients[1]
  coxfit40 = coxph(Surv(alltime40,del40)~Z40)
  influP40=coxfit40$coefficients[1]
  
  index20 = sample(which(del==1),numcol)
  index40 = sample(which(del40==1),numcol)
  
  for (i in 1:numcol){
    
    #smallest time -> large time
...
H40 = matrix(H40,ncol=numcol+1,byrow=TRUE)
P40 = matrix(P40,ncol=numcol+1,byrow=TRUE)

Hsum = colSums(H)
Hmean = Hsum/iterate_time
Hbias = (Hmean-Hmean[1])*N/c(1,1:numcol)

Psum = colSums(P)
Pmean = Psum/iterate_time
Pbias = (Pmean - Pmean[1])*N/c(1,1:numcol)

H40sum = colSums(H40)
H40mean = H40sum/iterate_time
H40bias = (H40mean-H40mean[1])*N/c(1,1:numcol)

P40sum = colSums(P40)
P40mean = P40sum/iterate_time
P40bias = (P40mean - P40mean[1])*N/c(1,1:numcol)


xx= 0:(numcol-19)
plot(xx,Hbias[1:(numcol-18)],col=1,type="b",pch=21,ylim=c(-50,0.5),xlab = "Number of outliers",ylab ="IF")
lines(xx,rep(0,(numcol-18)),type="l")
lines(xx,Pbias[1:(numcol-18)],col=2,type="b",pch=22)
lines(xx,H40bias[1:(numcol-18)],col=3,type="b",,pch=23)
lines(xx,P40bias[1:(numcol-18)],col=4,type="b",,pch=24)
legend("bottomright",x.intersp=0.4,y.intersp=0.2,pch=21:24,text.width=5,legend=c("MHDE (no censoring)","PMLE (no censoring)","MHDE (40% censoring)","PMLE(40% censoring)"),col=c(1,2,3,4),bty="n")
