...
for (i in 1:100){
  data0 = inversion_gen((N*9/5),0)  # 0-8
  cen0 = runif((N*9/5),0,12)  # random censoring
  ind0 = 1*(data0<cen0) # indicator I(t<c)
  sum(ind0)/(N*9/5)     #approximate censor rate
  newdata0 = rep(0,(N*9/5))
  for (i in 1:(N*9/5)){
    newdata0[i] = min(data0[i],cen0[i])
  }
  
  
  data1 = inversion_gen((N*3/5),1)  # 0-6
  cen1 = runif((N*3/5),0,6.5)
  ind1 = 1*(data1<cen1) # indicator I(t<c)
  #sum(ind1)/N     #approximate censor rate
  newdata1 = rep(0,(N*3/5))
  for (i in 1:(N*3/5)){
    newdata1[i] = min(data1[i],cen1[i])
  }
  
  
  data2 = inversion_gen((N*3/5),2)  # 0-4
  cen2 = runif((N*3/5),0,3.5)
  ind2 = 1*(data2<cen2) # indicator I(t<c)
  #sum(ind2)/N     #approximate censor rate
  newdata2 = rep(0,(N*3/5))
  for (i in 1:(N*3/5)){
