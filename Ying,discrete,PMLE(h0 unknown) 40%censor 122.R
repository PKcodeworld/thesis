...
for (i in 1:100){
  data0 = inversion_gen((N*3/5),0)  # 0-8
  cen0 = runif((N*3/5),0,6)  # random censoring
  ind0 = 1*(data0<cen0) # indicator I(t<c)
  sum(ind0)/(N*3/5)     #approximate censor rate
  newdata0 = rep(0,(N*3/5))
  for (i in 1:(N*3/5)){
    newdata0[i] = min(data0[i],cen0[i])
  }
  
  
  data1 = inversion_gen((N*6/5),1)  # 0-6
  cen1 = runif((N*6/5),0,3.3)
  ind1 = 1*(data1<cen1) # indicator I(t<c)
  #sum(ind1)/N     #approximate censor rate
  newdata1 = rep(0,(N*6/5))
  for (i in 1:(N*6/5)){
    newdata1[i] = min(data1[i],cen1[i])
  }
  
  
  data2 = inversion_gen((N*6/5),2)  # 0-4
  cen2 = runif((N*6/5),0,1.6)
  ind2 = 1*(data2<cen2) # indicator I(t<c)
  # sum(ind2)/N     #approximate censor rate
  newdata2 = rep(0,(N*6/5))
  for (i in 1:(N*6/5)){
  ...
  g = c(rep(0,(N*3/5)),rep(1,(N*6/5)),rep(2,(N*6/5)))
